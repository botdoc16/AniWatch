...
export default function Catalog() {
  ...
  const loadPopularAnime = async () => {
    try {
      setIsLoading(true);
      console.log('Начинаем загрузку всего списка аниме...');
      const popularAnimes = await animeApi.getPopularAnime(1, 2000); // Увеличиваем лимит до 2000
      console.log('Загружено аниме:', popularAnimes.animes.length);

      const favorites = user ? await favoritesService.getFavorites() : [];
      const animesWithFavorites = popularAnimes.animes.map(anime => ({
        ...anime,
        isFavorite: favorites.some(fav => fav.id === anime.id)
      }));

      console.log('Обработано аниме с избранным:', animesWithFavorites.length);
      setAnimes(animesWithFavorites);
      setFilteredAnimes(animesWithFavorites);
    } catch (error) {
